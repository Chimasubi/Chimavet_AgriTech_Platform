...
import { COMMODITIES } from '@/lib/agents'
...
export default function MarketScoutPage() {
  ...
  const load = async () => {
    setLoading(true)
    const r = await fetch('/api/agents/market-scout')
  ...
  const askAdvice = async (id, q) => {
    setSelected(id)
    const r = await fetch('/api/agents/market-scout', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ commodity: id, quantity: q })
    })
  ...
  return (
    <>
      <div className="mb-6 flex items-center justify-between flex-wrap gap-3">
        ...
        <button onClick={load} className="flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-full text-sm font-medium hover:bg-emerald-700">
      ...
      {advice && (
        <div className={`bg-white rounded-2xl border-2 p-6 fade-in ${
          advice.action === 'sell' ? 'border-emerald-500' :
          advice.action === 'hold' ? 'border-amber-500' : 'border-stone-300'}`}>
          <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
            ...
            <div className={`text-2xl font-black ${
              advice.action === 'sell' ? 'text-emerald-600' :
              advice.action === 'hold' ? 'text-amber-600' : 'text-stone-600'}`}>
              {advice.verdict}
            </div>
          </div>
          ...
          <div className="grid sm:grid-cols-3 gap-3 text-sm">
            <div className="bg-stone-50 p-3 rounded-lg">
              ...
              <div className="font-bold">{TSh(advice.item.current)}</div>
            </div>
            <div className="bg-stone-50 p-3 rounded-lg">
              <div className="text-xs text-stone-500">Est. revenue</div>
              <div className="font-bold text-emerald-700">{TSh(advice.estimatedRevenue)}</div>
            </div>
            <div className="bg-stone-50 p-3 rounded-lg">
              <div className="text-xs text-stone-500">Best market</div>
              <div className="font-bold">{advice.item.market}</div>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="mt-6 bg-blue-50 border border-blue-200 rounded-2xl p-4 text-xs text-blue-900">
        <strong>How it works:</strong> Prices aggregated from major markets across the region. The advisor analyzes 14-day price trends to recommend whether to sell now, hold, or monitor. Connect a real market data API (e.g. FAO, Esoko, or a regional commodity exchange) for live production data.
      </div>
    </>
  )
}
