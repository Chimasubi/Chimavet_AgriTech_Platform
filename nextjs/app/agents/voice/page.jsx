...
import { VOICE_LANGS, getVoiceResponse } from '@/lib/agents'
  ...
  return (
    <>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 bg-white rounded-2xl border border-stone-200 flex flex-col h-[60vh]">
          ...
          <div className="p-3 border-t flex gap-2">
            <input value={text} onChange={e => setText(e.target.value)} onKeyDown={e => e.key === 'Enter' && handle(text)}
              placeholder={lang.startsWith('sw') ? 'Andika swali lako hapa...' : 'Type your question...'}
              className="flex-1 px-4 py-2 border border-stone-300 rounded-full text-sm focus:outline-none focus:border-emerald-500" />
        ...
        <div className="space-y-3">
          <button onClick={listening ? stopListen : startListen} disabled={!supported}
            className={`w-full aspect-square rounded-2xl text-white font-bold text-lg flex flex-col items-center justify-center gap-2 transition shadow-lg ${
              listening ? 'bg-red-500 animate-pulse' : 'bg-violet-600 hover:bg-violet-700'} ${!supported ? 'opacity-50 cursor-not-allowed' : ''}`}>
            {listening ? <><MicOff className="w-16 h-16" /> Listening...</> : <><Mic className="w-16 h-16" /> {supported ? 'Tap to Speak' : 'Not Supported'}</>}
          </button>
          <button onClick={speaking ? stopSpeak : () => speak(msgs.filter(m => m.role === 'bot').pop()?.text || '')}
            className={`w-full py-3 rounded-xl font-medium flex items-center justify-center gap-2 ${speaking ? 'bg-amber-100 text-amber-700' : 'bg-stone-100 text-stone-700 hover:bg-stone-200'}`}>
            {speaking ? <><VolumeX className="w-4 h-4" /> Stop Speaking</> : <><Volume2 className="w-4 h-4" /> Replay Last</>}
          </button>
          <div className="bg-violet-50 border border-violet-200 rounded-xl p-3 text-xs text-violet-900">
            ...
            <div className="mt-2 space-y-1">
              {['Bei ya mahindi', 'Mbuzi', 'Umwagiliaji', 'Mbolea', 'Kuku'].map(p => (
                <button key={p} onClick={() => handle(p)} className="block w-full text-left px-2 py-1 hover:bg-violet-100 rounded">
                  🎤 "{p}"
                </button>
              ))}
            </div>
      ...
      <div className="mt-6 bg-blue-50 border border-blue-200 rounded-2xl p-4 text-xs text-blue-900">
        <strong>Browser support:</strong> Speech recognition works in Chrome, Edge, and Safari. On unsupported browsers, the text input still works. Works best on HTTPS or localhost.
      </div>
    </>
  )
}
