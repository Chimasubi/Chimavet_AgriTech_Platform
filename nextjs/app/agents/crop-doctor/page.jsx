...
export default function CropDoctorPage() {
  ...
  const analyze = async () => {
    ...
    setLoading(true)
    try {
      const res = await fetch('/api/agents/crop-doctor', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ filename: image.name, fileSize: image.size, context })
      })
      const data = await res.json()
      setResult(data.result)
    } catch {
      setResult({ name: 'Error', symptoms: [], treatment: ['Could not analyze image. Try again.'], severity: 'Unknown' })
    }
    setLoading(false)
  }
  ...
  return (
    <>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl border border-stone-200 p-6">
          ...
          <div onClick={() => fileRef.current?.click()}
            ...
            {preview ? (
              <img src={preview} alt="crop" className="max-h-full max-w-full object-contain rounded-lg" />
            ) : (
              <>
                <Upload className="w-12 h-12 text-stone-400 mb-2" />
                <p className="font-medium text-stone-600">Click to upload</p>
                <p className="text-xs text-stone-400 mt-1">JPG, PNG · clear close-up of affected leaf</p>
              </>
            )}
          </div>
          ...
          <div className="mt-4">
            ...
            <textarea value={context} onChange={e => setContext(e.target.value)} placeholder="e.g. yellow spots on tomato leaves, started 3 days ago after rain..." rows={3}
          ...
          <button onClick={analyze} disabled={!image || loading}
            className="mt-4 w-full py-3 bg-emerald-600 text-white rounded-xl font-semibold hover:bg-emerald-700 disabled:bg-stone-300 disabled:cursor-not-allowed flex items-center justify-center gap-2">
        ...
        <div>
          {!result && !loading && (
            <div className="bg-white rounded-2xl border border-stone-200 p-12 text-center text-stone-500 h-full flex flex-col items-center justify-center">
              ...
              <p className="text-xs mt-1">Works best with clear, well-lit close-ups of the affected area</p>
            </div>
          )}
          {result && (
            <div className={`bg-white rounded-2xl border-2 ${sevColor(result.severity)} p-6 fade-in`}>
              ...
              {result.severity === 'Critical' && (
                <div className="mt-4 p-3 bg-red-100 border border-red-200 rounded-lg text-xs text-red-800">
                  ⚠️ Critical — act within 24-48 hours to prevent spread to other plants.
                </div>
              )}
            </div>
          )}
      ...
      <div className="mt-6 bg-blue-50 border border-blue-200 rounded-2xl p-4 text-xs text-blue-900">
        <strong>How it works:</strong> Photos are analyzed by an AI vision model trained on thousands of crop disease images. For best results: take close-up photos in daylight, include both healthy and affected parts of the plant, avoid blurry images.
      </div>
    </>
  )
}
