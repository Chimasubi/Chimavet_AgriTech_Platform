        ...
        <div className="fixed bottom-6 right-6 z-50 w-full max-w-md bg-white rounded-3xl shadow-2xl border border-slate-200 flex flex-col overflow-hidden h-[580px] animate-in fade-in slide-in-from-bottom-5">
          ...
          <div className="bg-gradient-to-r from-slate-950 via-green-950 to-slate-900 text-white p-4 flex items-center justify-between border-b border-slate-800">
            <div className="flex items-center gap-3">
              <div className="p-2.5 bg-green-500/20 text-green-400 rounded-2xl backdrop-blur-md">
                <Bot className="w-5 h-5" />
              </div>
              <div>
                <h3 className="font-extrabold text-sm flex items-center gap-1.5">
                  ChimaAI Agronomy Assistant
                  <Badge className="bg-green-500/20 text-green-300 text-[10px] border-none px-1.5 py-0.2">v2.4 Pro</Badge>
                </h3>
                <span className="text-[11px] text-slate-400 flex items-center gap-1">
                  <span className="w-1.5 h-1.5 bg-green-400 rounded-full inline-block animate-pulse"></span> Tanzania Crop Advisory Active
                </span>
              </div>
            </div>
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-4 bg-slate-50/70 custom-scrollbar">
            {messages.map((msg) => (
              ...
              >
                {msg.sender === "ai" && (
                  <div className="w-8 h-8 rounded-full bg-green-700 text-white flex items-center justify-center shrink-0 text-xs font-black shadow-sm">
                    AI
                  </div>
                )}
                ...
                >
                  {msg.category && (
                    <span className="text-[10px] font-extrabold uppercase tracking-wider text-green-800 bg-green-100/80 px-2 py-0.5 rounded-md inline-block mb-2">
                      {msg.category}
                    </span>
                  )}
            ...
            {isTyping && (
              <div className="flex items-center gap-2 text-slate-400 text-xs font-semibold pl-10">
                <Bot className="w-4 h-4 animate-bounce text-green-600" />
            ...
            <div className="p-2.5 bg-white border-t border-slate-100 flex gap-1.5 overflow-x-auto custom-scrollbar">
              {suggestedPrompts.map((prompt, i) => (
                <button
                  ...
                  className="text-xs bg-slate-100 hover:bg-green-50 hover:text-green-800 text-slate-700 px-3 py-1.5 rounded-full whitespace-nowrap border border-slate-200 transition-colors shrink-0 font-medium"
                >
                  💡 {prompt}
                </button>
              ))}
          ...
          <div className="p-3 bg-white border-t border-slate-200 flex gap-2">
            <Input
              ...
              className="flex-1 h-11 border-slate-200 text-xs sm:text-sm rounded-xl focus-visible:ring-green-600"
            />
            <Button
              onClick={() => handleSend()}
              disabled={!inputValue.trim() || isTyping}
              className="bg-green-600 hover:bg-green-700 h-11 w-11 p-0 rounded-xl shadow-md"
            >
