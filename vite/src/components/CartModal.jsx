...
export default function CartModal({ open, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col fade-in">
        ...
        {cart.length > 0 && (
          <div className="p-5 border-t bg-stone-50 rounded-b-2xl">
            <div className="flex justify-between text-sm mb-1"><span>Subtotal</span><span className="tabular-nums">{TSh(total)}</span></div>
            <div className="flex justify-between text-sm mb-3 text-stone-500"><span>Delivery</span><span>Calculated at checkout</span></div>
            <div className="flex justify-between font-bold text-lg mb-4"><span>Total</span><span className="tabular-nums">{TSh(total)}</span></div>
            <div className="flex gap-2">
              <button onClick={onClose} className="flex-1 py-2.5 border border-stone-300 rounded-lg text-sm font-medium hover:bg-stone-100">Continue</button>
              <button onClick={() => { alert(`Agiza limekubaliwa! Total ${TSh(total)}`); clearCart(); onClose() }} className="flex-1 py-2.5 bg-emerald-600 text-white rounded-lg text-sm font-medium hover:bg-emerald-700">Lipia Sasa</button>
            </div>
          </div>
        )}
      </div>
